...
export const GET = withAdminAuth(async (request: NextRequest) => {
  try {
    ...
    if (!healthCheck.healthy) {
      ...
      if (healthCheck.responseTime && healthCheck.responseTime > 1000) healthScore -= 20
      if (healthCheck.poolStatus && healthCheck.poolStatus.waitingClients > 5) healthScore -= 15
    ...
    const response = {
      ...
      connectionPool: healthCheck.healthy && healthCheck.poolStatus ? {
        totalConnections: healthCheck.poolStatus.totalConnections,
        idleConnections: healthCheck.poolStatus.idleConnections,
        waitingClients: healthCheck.poolStatus.waitingClients,
        utilizationPercentage: Math.round((healthCheck.poolStatus.totalConnections - healthCheck.poolStatus.idleConnections) / healthCheck.poolStatus.totalConnections * 100)
      } : null,
      queryPerformance: queryMetrics ? {
        totalQueries: queryMetrics.totalQueries,
        averageDuration: queryMetrics.avgDuration,
    ...
    if (healthCheck.healthy) {
      ...
      if (healthCheck.responseTime && healthCheck.responseTime > 1000) {
        recommendations.push({
          type: 'performance',
          priority: 'high',
          message: `Database response time is ${healthCheck.responseTime}ms. Consider optimizing queries or checking network latency.`
        })
      }
      
      if (healthCheck.poolStatus && healthCheck.poolStatus.waitingClients > 5) {
        recommendations.push({
          type: 'capacity',
          priority: 'high',
          message: `${healthCheck.poolStatus.waitingClients} clients waiting for connections. Consider increasing pool size.`
        })
      }
