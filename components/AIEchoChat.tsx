    ...
    try {
      const response = await fetch('/api/ai-echo/chat', {
        ...
        body: JSON.stringify({
          ...
          isDemo,
          conversationId,
          includeVoice: true, // Enable voice synthesis for AI responses
      ...
      if (response.ok) {
        ...
        setTimeout(async () => {
          ...
          const aiMessage: ChatMessage = {
            ...
            audioLoading: !data.voice?.audioUrl, // Only show loading if no voice returned
            audioUrl: data.voice?.audioUrl, // Use voice from API response
            voiceQuality: data.voice?.quality,
            generationTime: data.voice?.generationTime
          }

          setMessages(prev => [...prev, aiMessage])
          setTrainingData(data.trainingData)
          
          // If voice was not included in API response, fall back to manual synthesis
          if (!data.voice?.audioUrl) {
            await synthesizeVoice(messageId, data.response)
          } else {
            // Auto-play if enabled and voice was returned from API
            if (voiceSettings.autoPlay) {
              playAudio(messageId, data.voice.audioUrl)
            }
          }
