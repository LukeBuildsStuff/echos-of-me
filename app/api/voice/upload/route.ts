...
export async function POST(request: NextRequest) {
  try {
    ...
    // Enhanced voice cloning integration
    let trainingStatus = null
    let integrationStatus = null
    let voiceDatasetInfo = null

    if (isComplete) {
      try {
        // Create voice training dataset using the architecture
        const dataset = await voiceCloneArchitecture.createVoiceTrainingDataset(userId)
        voiceDatasetInfo = {
          totalDuration: dataset.totalDuration,
          qualityScore: dataset.qualityScore,
          phoneticCoverage: dataset.phoneticCoverage,
          emotionalRange: dataset.emotionalRange,
          isTrainingReady: dataset.isComplete
        }

        if (shouldTriggerTraining && dataset.isComplete) {
          console.log(`Starting RTX 5090 voice clone training for user ${userId}`)
          
          // Start voice clone training with RTX 5090 optimization
          ...
          if (trainingResult.success) {
            trainingStatus = {
              ...
              message: 'RTX 5090 voice training initiated successfully',
              optimizations: 'XTTS-v2 with Flash Attention 2 and Tensor Core acceleration'
            }

            // Check voice-LLM integration readiness
            try {
              const integrationReadiness = await voiceLLMIntegrator.getIntegrationStatus(userId)
              
              if (integrationReadiness.integrationStatus === 'ready') {
                // Start integrated voice + LLM training
                const integrationResult = await voiceLLMIntegrator.integrateVoiceWithLLM(
                  userId, 
                  voiceCloneId
                )
                ...
                integrationStatus = {
                  success: false,
                  message: `Voice training started. ${integrationReadiness.message}`,
                  readiness: integrationReadiness
                }
              }
            } catch (integrationError) {
              console.warn('Voice-LLM integration check failed:', integrationError)
              integrationStatus = {
                success: false,
                message: 'Voice training started. Complete text AI training for full integration.'
              }
            }
          } else {
            trainingStatus = {
              ...
              message: 'Voice training failed to start. Please try again.'
            }
          }
        }
      } catch (datasetError) {
        console.error('Voice dataset creation failed:', datasetError)
        voiceDatasetInfo = {
          error: 'Failed to analyze voice recordings',
          suggestion: 'Please ensure all recordings are clear and meet quality requirements'
        }
      }
    }
    ...
    try {
      ...
      if (mlResponse.ok) {
        ...
        return NextResponse.json({
          ...
          voiceCloning: {
            dataset: voiceDatasetInfo,
            training: trainingStatus,
            integration: integrationStatus,
            rtx5090Optimized: true,
            nextSteps: isComplete ? 
              (trainingStatus?.status === 'training_started' ? 
                ['RTX 5090 training in progress', 'Voice clone will be ready in 2-3 minutes', 'Complete text AI training for full voice integration'] :
                ['Voice recordings complete', 'Ready to start RTX 5090 training', 'High-quality voice clone with XTTS-v2']
              ) : 
              [`Record ${expectedPassages.length - completedPassages.length} more passages`, 'Each passage captures different voice characteristics', 'Optimal recording: quiet environment, clear speech']
          }
        })
      } else {
