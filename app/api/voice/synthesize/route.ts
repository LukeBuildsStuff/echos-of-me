...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      // Try local voice cloning architecture first
      try {
        const voiceRequest = {
          text: text,
        ...
        console.log('Attempting local RTX 5090 voice synthesis...')
        const localResult = await voiceCloneArchitecture.synthesizeVoice(voiceRequest)
        
        return NextResponse.json({
          success: true,
          audioUrl: localResult.audioUrl,
          voiceId: localResult.modelUsed,
          duration: localResult.duration,
          generationTime: localResult.generationTime,
          quality: localResult.quality,
          source: 'rtx5090_local',
          message: `High-quality voice synthesis using RTX 5090 (${localResult.generationTime}ms)`
        })
        
      } catch (localError) {
        console.log('Local voice synthesis failed, falling back to ML service:', localError.message)
      }

      // Fallback to ML inference service for voice synthesis
