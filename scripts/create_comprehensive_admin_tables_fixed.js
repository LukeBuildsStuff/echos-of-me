...
async function createComprehensiveAdminTables() {
  ...
  try {
    ...
    // 2. Family relationships and structure (using INTEGER foreign keys for users)
    await client.query(`
    ...
    // Add foreign key constraints and enhance users table with proper UUID reference
    await client.query(`
      ALTER TABLE users 
      ...
      ADD COLUMN IF NOT EXISTS memorial_contact_id INTEGER REFERENCES users(id) ON DELETE SET NULL,
