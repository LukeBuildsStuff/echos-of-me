...
`
  ...
  async generateResponse(request: InferenceRequest): Promise<InferenceResponse> {
    ...
    try {
      ...
      // Get deploymentId from the method scope or use modelId as fallback
      let errorDeploymentId = modelId
      try {
        // Try to get deploymentId if it was set in the try block
        errorDeploymentId = await this.getOrDeployUserModel(userId)
      } catch {
        // If that fails, use modelId
        errorDeploymentId = modelId || 'unknown'
      }
